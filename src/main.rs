...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Ticket => {
            ...
            println!("{}", ticket);
            println!("\n在其他设备上运行以下命令来连接:");
            println!("cargo run -- connect {}", ticket);
        }
        Commands::Auto => {
            let network = NetworkManager::new(cli.name.clone()).await?;
    ...
    tokio::spawn(async move {
        while let Some(message) = message_receiver.recv().await {
            ...
            match &message.content {
                ...
                network::ClipboardContent::Image { width, height, data } => {
...
/// 连接到其他设备
async fn connect_to_peer(clipboard: ClipboardManager, network: NetworkManager, ticket: &str) -> Result<()> {
    let notifier = NotificationManager::new();
